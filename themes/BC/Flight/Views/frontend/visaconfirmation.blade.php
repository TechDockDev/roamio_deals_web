...
<div class="container card mt-3 pt-3">
  ...
  <div class="card mt-3 pt-3" style="background:#FAFAFA;">
    ...
    <form method="POST" name="customerData" action="{{ route('payment.visa.form') }}">
      @csrf
        @php
          $merchant_id =49578;
        @endphp
      ...
      <div class="row py-3">
          ...
          <input type="hidden" name="visa_id" value="{{ $confirm_visa->id }}">
          <input type="hidden" name="user_id" value="@if(auth()->user()){{auth()->user()->id}}@endif">
          <input type="hidden" name="firstname" value="{{ $confirm_visa->firstname }}">
          <input type="hidden" name="email" value="{{ $confirm_visa->email }}">
          <input type="hidden" name="passport_no" value="{{ $confirm_visa->passport_no }}">
          <input type="hidden" name="passport_expiry" value="{{ $confirm_visa->passport_expiry }}">
