    ...
    public function search($request)
    {
        ...
        if (is_array($terms) and !empty($terms = array_filter(array_values($terms)))) {
            foreach ($terms as $index=>$termId){
                $model_event->join('bravo_event_term as tt'.$index, function($join) use ($termId,$index){
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        switch ($orderby){
            ...
            default:
                if(!empty($request['order']) and !empty($request['order_by'])){
                    $model_event->orderBy("bravo_events.".$request['order'], $request['order_by']);
                }else{
        ...
        return $model_event->with(['location','hasWishList','translation']);
    }
