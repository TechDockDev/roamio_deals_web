...
class Hotel extends Bookable
{
    ...
    public function getReviewDataAttribute()
    {
      ...
      $list_score['rate'] = array_values($list_score['rate']);
    ...
    public function search($request)
    {
        ...
        if(!empty($lat = $request["map_lat"] ?? "") and !empty($lgn = $request["map_lgn"] ?? "") and !empty($request["map_place"] ?? ""))
        {
            ...
            $distance  = setting_item('hotel_location_radius_value',0);
            if(!empty($distance) and setting_item('hotel_location_search_style')=='autocompletePlace'){
                $distanceType = setting_item('hotel_location_radius_type',3959);
