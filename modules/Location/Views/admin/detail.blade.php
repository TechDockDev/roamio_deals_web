...
@section('content')
    <form action="{{route('location.admin.store',['id'=>($row->location_id) ? $row->location_id : '-1','lang'=>request()->query('lang')])}}" method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->location_id ? 'Edit: '.$row->name : __("Add new location")}}</h1>
                    @if($row->slug)
